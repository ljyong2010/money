...
$.fn.ProvinceCity = function(){
	var _self = this;
	//定义3个默认值
	 _self.data("province",["河北", "河北"]);
	 _self.data("city1",["", ""]);
	 _self.data("city2",["", ""]);
	//插入3个空的下拉框
	_self.append("<select name='sheng' style='width: 65px'></select>");
	_self.append("<select name='shi' style='width: 65px'></select>");
	_self.append("<select name='xian' style='width: 65px'></select>");
	//分别获取3个下拉框
	...
	$sel2.change(function(){
		...
		$.each( GC[9][index2-1] , function(index,data){
			//$sel3.append("<option value='"+_self.data("city2")[1]+"'>"+_self.data("city2")[0]+"</option>");
			$sel3.append("<option value='"+data+"'>"+data+"</option>");
		})
	});
