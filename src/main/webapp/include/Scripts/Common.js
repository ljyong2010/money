...
$(function () {
    $("#btnBack").click(function () { $(this).attr("disabled", true); history.go(-1); });
    $("#btnClose").click(function () { closeLayerWindow(); });
});
...
function formPost(url, callback) {
    if ($.isFunction(url))
        ajaxPost($("#webform").attr("action"), $("#webform").serializeArray(), function (data) { url(data); });
    else if (typeof url == "string")
        ajaxPost(url, $("#webform").serializeArray(), function (data) { callback(data); });
    else if (typeof url == "object")
        ajaxPost(!!url.url ? url.url : $("#" + url.formid).attr("action"), $("#" + url.formid).serializeArray(), function (data) { url.callback(data); });
    else
        return;
}
function ajaxPost(url, data, fnCallback) {
    return $.ajax({
        ...
        "dataType": "json",
        "processData": true,
        "success": function (data) { try { if (data.code == -2) { alert(data.msg); top.location.href = "/"; return; } } catch (e) {; } fnCallback(data); },
        "error": function (r) { try { if (!!layer) layer.alert(r.statusText + "!" + r.responseText); else alert(r.statusText + "!" + r.responseText); } catch (e) {; } if (r.statusText === "Server Too Busy") { location.href = "/"; } }
    });
}
function getTree(treeid) { return $.fn.zTree.getZTreeObj(treeid); }
...
function bindSelect(selector, url, data, txtField, valField, callback) {
    ajaxPost(url, data, function (d) {
        if (d.data) { $.each(d.data, function (i, v) { $('<option></option>').text(v[txtField]).val(v[valField]).appendTo($(selector)); }); }
