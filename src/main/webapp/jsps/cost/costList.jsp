...
<div class="pd-20">
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-10">
        <span class="l"></span>
        <span class="r"><a id="btnNew" onclick="openWinFull('${ctx}/tcost?pindex=addCost','新增',640, 600);" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加</a></span>
    </div>
    <div class="cl">
...
<script>
    ...
    function bindData() {
        oTable = $("#datalist").dataTable({
            "sAjaxSource": "${ctx}/tcost/costList",
            "columns": [
                { "data": "costDetails" },
                { "data": "money" },
                { "data": "person" },
                { "data": null, "sClass": "text-c","mRender": function (data) { return data.date==null?"-":data.date.substring(0,10);} },
    ...
    function Btns(data) {
        var btns = ['<a onclick="openWin(\'${ctx}/tcost?pindex=addCost&id=' + data.ID + '\',\'查看\',640, 500);\" class="btn-link">查看</a>','<a name="edit" onclick="openWinFull(\'${ctx}/tcost?pindex=addCost&id=' + data.ID + '\',\'修改信息\',640, 500);\" class="btn-link">修改</a>', '<a href="javascript:delAsk(\'' + data.ID + '\');" class="btn-link">删除</a>'];
        return btns.join('&nbsp; ');
    }
    function delAsk(ID) {
        layer.confirm("确认删除吗？", function () {
            ajaxPost("${ctx}/tcost/delete", {
                id:ID
            }, function (d) {
                if (d.code > 0) {
