...
<div class="pd-20 deform">
    <form id="webform" action="${ctx}/twork/emplaySave" class="form form-horizontal">
        <input type="hidden" id="hID" NAME="ID" value="${param.get("id")}">
        <div class="row cl border-tg">
            <label class="form-label col-xs-3"><span class="c-red">*</span>员工姓名：</label>
            <div class="formControls col-xs-3">
                <input type="text" id="txtemployee" name="employee" class="input-text " datatype="*" nullmsg="必须填写姓名！" />
            </div>
            <label class="form-label col-xs-3">工资：</label>
            <div class="formControls col-xs-3">
                <input type="text" id="txtsalary" name="salary" class="input-text " datatype="*" nullmsg="必须填工资！" />
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-3">开始日期：</label>
            <div class="formControls col-xs-3">
                <input type="text" id="txtstartDate" name="startDate" class="input-text " datatype="*" nullmsg="必须填写开始日期！" value="" onclick="laydate()" />
            </div>
            <label class="form-label col-xs-3">结束日期：</label>
            <div class="formControls col-xs-3">
                <input type="text" id="txtendDate" name="endDate" class="input-text " datatype="*" nullmsg="必须填写结束日期！" value="" onclick="laydate()" />
            </div>
        </div>
    </form>
</div>
...
<script>
    var wForm = null;
    $(function () {
        ajaxPost("${ctx}/twork/getEmplay", {ID: $("#hID").val()}, function (d) {
            formAssign(d);
        });
